...
FLASH_Status FLASH_ErasePage(uint32_t Page_Address)
{
  ...
  { 
    ...
    /* if the previous operation is completed, proceed to erase the page */
	//DBG_print("\r\n %s:CR_PER_Set", __func__);
    FLASH->CR|= CR_PER_Set;
    //DBG_print("\r\n %s:Page_Address=%X", __func__, Page_Address);
    FLASH->AR = Page_Address; 
    //DBG_print("\r\n %s:CR_STRT_Set", __func__);
    FLASH->CR|= CR_STRT_Set;
...
{ 
  ...
  //DBG_print("\r\n %s:F:Timeout=%X", __func__, Timeout);
  if(Timeout == 0x00 )
  {
